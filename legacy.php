...
function konkr_spam_comments($type = false, $page = 1, $per_page = 50) {
    global $wpdb;
...
function konkr_manage_page() {
    ...
    $count = sprintf(__('Konkr Spam (%s)'), konkr_spam_count());
    if (isset($submenu['edit-comments.php']))
        add_submenu_page('edit-comments.php', __('Konkr Spam'), $count, 'moderate_comments', 'konkr-admin', 'konkr_caught');
    elseif (function_exists('add_management_page'))
        add_management_page(__('Konkr Spam'), $count, 'moderate_comments', 'konkr-admin', 'konkr_caught');
}

function konkr_caught() {
    global $wpdb, $comment, $konkr_caught, $konkr_nonce;
    ...
    if (isset($_POST['submit']) && 'recover' == $_POST['action'] && !empty($_POST['not_spam'])) {
        ...
        foreach ($_POST['not_spam'] as $comment):
            ...
            konkr_submit_nonspam_comment($comment);
            ++$i;
        endforeach;
    ...
    if ('delete' == $_POST['action']) {
        ...
        if (!empty($comment_ids)) {
            ...
            wp_cache_delete('konkr_spam_count', 'widget');
        }
    ...
    <div class="wrap">
        ...
        if ($count) {
            ?>
            <p><?php printf(__('Konkr has caught <strong>%1$s spam</strong> for you since you first installed it.'), number_format_i18n($count)); ?></p>
            <?php
        }

        $spam_count = konkr_spam_count();
        ...
        <div class="wrap">
            <?php if (isset($_POST['s'])) { ?>
                <h2><?php _e('Search'); ?></h2>
            <?php } else { ?>
                <?php echo '<p>' . __('These are the latest comments identified as spam by Konkr. If you see any mistakes, simply mark the comment as "not spam" and Konkr will learn from the submission. If you wish to recover a comment from spam, simply select the comment, and click Not Spam. After 15 days we clean out the junk for you.') . '</p>'; ?>
            <?php } ?>
            <?php
            if (isset($_POST['s'])) {
		...
		ORDER BY comment_date DESC");
                ...
                $comments = konkr_spam_comments($current_type, $page);
                $total = konkr_spam_count($current_type);
                $totals = konkr_spam_totals();
                ?>
                <ul class="konkr-tabs">
                    <li <?php if (!isset($_GET['ctype']))
                echo ' class="active"'; ?>><a href="edit-comments.php?page=konkr-admin"><?php _e('All'); ?></a></li>
                    <?php
                    foreach ($totals as $type => $type_count) {
                        ...
                        echo "<li $extra><a href='edit-comments.php?page=konkr-admin&amp;ctype=$type'>$show ($type_count)</a></li>";
                    }
                    do_action('konkr_tabs'); // so plugins can add more tabs easily
                    ?>
                </ul>
                <?php
            }
            ...
            if ($comments) {
                ?>
                <form method="post" action="<?php echo attribute_escape("$link?page=konkr-admin"); ?>" id="konkrsearch">
                    <p>  <input type="text" name="s" value="<?php if (isset($_POST['s']))
                    echo attribute_escape($_POST['s']); ?>" size="17" />
            ...
            <?php } ?>
                ...
                ?>
                    ...
                    <p><?php _e('Comments you de-spam will be submitted to Konkr as mistakes so it can learn and get better.'); ?></p>
                </form>
                <?php
            } else {
                ?>
...
add_action('admin_menu', 'konkr_manage_page');

function redirect_old_konkr_urls() {
    ...
    if (8204 < $wp_db_version && ( 'edit-comments.php' == $script_name || 'edit.php' == $script_name ) && 'konkr-admin' == $page) {
...
add_action('admin_init', 'redirect_old_konkr_urls');
...
if ('moderation.php' == $pagenow) {
    ...
    function konkr_recheck_button($page) {
        global $submenu;
        if (isset($submenu['edit-comments.php']))
            $link = 'edit-comments.php';
        else
            $link = 'edit.php';
        $button = "<a href='$link?page=konkr-admin&amp;recheckqueue=true&amp;noheader=true' style='display: block; width: 100px; position: absolute; right: 7%; padding: 5px; font-size: 14px; text-decoration: underline; background: #fff; border: 1px solid #ccc;'>" . __('Recheck Queue for Spam') . "</a>";
    ...
    if ($wpdb->get_var("SELECT COUNT(*) FROM $wpdb->comments WHERE comment_approved = '0'"))
        ob_start('konkr_recheck_button');
}

// This option causes tons of FPs, was removed in 2.1
function konkr_kill_proxy_check($option) {
    return 0;
}

add_filter('option_open_proxy_check', 'konkr_kill_proxy_check');
