jQuery(document).ready(function () {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                if (response) {
                    ...
                    jQuery("#author_comment_url_"+ thisId).attr('cid', thisId).addClass('konkr_undo_link_removal').html('<span>URL removed (</span>undo<span>)</span>');
                }
            }
        });
    ...
    jQuery('.konkr_undo_link_removal').live('click', function () {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                if (response) {
                    ...
                    // Show link
                    jQuery("#author_comment_url_"+ thisId).removeClass('konkr_undo_link_removal').html(thisUrl);
                }
            }
        });
    ...
    jQuery('a[id^="author_comment_url"]').mouseover(function () {
        ...
        if (jQuery(this).find('.mShot').length == 0 && !jQuery(this).hasClass('konkr_undo_link_removal')) {
